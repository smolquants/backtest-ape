...
/// @title Uniswap V3 Liquidity Provider Fee Backtester
/// @notice Backtests an LP position in pool reporting accumulated fees (token0, token1) values of LP tokens
contract UniswapV3LPFeeBacktest is UniswapV3LPBacktest {
    constructor(address _manager) UniswapV3LPBacktest(_manager) {}

    /// @notice Reports the accumulated fee (token0, token1) values of the LP tokens owned by this contract
    /// @return values_ The current (token0, token1) accumulated fees of the LP tokens owned by this contract
    function values() public view virtual override returns (uint256[] memory values_) {
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            ...
            values_[0] += amount0Fee;
            values_[1] += amount1Fee;
        }
    }
}
